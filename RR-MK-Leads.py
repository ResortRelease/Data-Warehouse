...
def clean_source(contacts, dispos):  
  bar.next()
  ...
  # Clean phones
  contacts_sms['HomePhone'] = contacts_sms['HomePhone'].apply(rr_fun.clean_number)
  contacts_sms['SecondaryPhone'] = contacts_sms['SecondaryPhone'].apply(rr_fun.clean_number)

  # Clean emails
  contacts_email['EmailAddress'] = contacts_email['EmailAddress'].apply(rr_fun.clean_email)
  ...
  contacts = contacts[["dealid", "status", "EmailAddress", "HomePhone", "SecondaryPhone", "Brand", "source", "SubSource", "datecr", "dateASAP", "salesdate", "cancelsale", "utm_term", "utm_campaign", "utm_source", "utm_medium", "utm_content", "hearduson", "total sold", "days", "was sold", "hasform", "hasapp", "State"]]
...
contacts_raw = pd.read_csv(contacts['Body'], index_col=False, low_memory=False, encoding="ISO-8859-1")

dispos = s3.get_object(Bucket=BUCKET_NAME, Key='DUMP/Dispo1.csv')
dispos = pd.read_csv(dispos['Body'], index_col=False, low_memory=False, encoding="ISO-8859-1")

clean_source(contacts, dispos)
bar.finish()